...
def rand_graphs(n, nP):
    iters = 10000000
    for i in range(iters):
        if i % 1000 == 0:
            print("      " + str(i))
        r = random.random() * .1 + .45
        G = Graph(n)
        for v in range(n):
            for w in range(v):
                if random.random() < r:
...
#                if v != w:
#                    G.add_edge(v, w)
#        for v in range(n - nP, n):
#            for w in range(n - nP, n):
#                if v != w:
#                    G.del_edge(v, w)
        yield(G)
...
#            break
    ...
    for nP in range(5, max_n + 1):
        nT = 10
        #while not any(all(induced_subgraph_isomorphism(P, T) for P in patterns[nP]) for T in patterns[nT]):
        while nP > nT or not any(all(induced_subgraph_isomorphism(P, T) for P in patterns[nP]) for T in rand_graphs(nT, nP)):
            nT += 1
            print("   ", nT)
        print(nP, nT)
